...
if ($assembly.GetName().Version.ToString() -cne '1.8.1.0') {
    throw 'Compiled application version is not 1.8.1.0.'
}
$company = @(
...
if (-not $manifestUrl.EndsWith('/update-v2.json', [StringComparison]::Ordinal) -or
    ...
    throw 'Compiled 1.8.1 application is not pinned to the v2 update channel.'
}
...
if ([string]$v2.version -cne '1.8.1' -or
    [long]$v2.size -ne [long]$installer.Length -or
    [string]$v2.sha256 -cne $installerHash -or
    [string]$v2.installerUrl -cne 'https://raw.githubusercontent.com/alosev394-ai/MajesticBoost/main/dist/MajesticBoost-Setup-1.8.1.exe') {
