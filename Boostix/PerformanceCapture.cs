...
namespace Boostix
{
    ...
    internal sealed class PerformanceTargetProcess
    {
    ...
    internal static class PerformanceCaptureService
    {
        ...
        private const long MinimumTargetWorkingSetBytes = 32L * 1024L * 1024L;
        private static readonly HashSet<string> ExcludedTargetProcesses =
            new HashSet<string>(
                new[]
                {
        ...
        public static PerformanceTargetProcess FindRunningTarget()
        {
        ...
        public static Task<PerformanceCaptureAttemptResult> CaptureRunningTargetAsync(
            ...
            CancellationToken cancellationToken)
        {
            PerformanceTargetProcess target = FindRunningTarget();
            if (target == null)
            {
                return Task.FromResult(CreateFailure(
                    PerformanceCaptureStatus.NoSupportedProcess,
        ...
        public static async Task<PerformanceCaptureAttemptResult> CaptureAsync(
            ...
            CancellationToken cancellationToken)
        {
            if (target == null || !IsSameTargetProcessRunning(target))
            {
                return CreateFailure(
                    PerformanceCaptureStatus.TargetExited,
                    "Выбранное приложение уже закрыто или было перезапущено.",
                    target,
                    elevated,
                    null,
                    null);
            ...
            if (elevated && !IsTrustedInstalledToolPath(tool.Path))
            {
                return CreateFailure(
                    ...
                    "Повтор с UAC доступен только для проверенного компонента из папки установки приложения.",
                    target,
                    true,
                    tool.Path,
                    null);
            ...
            if (!IsSameTargetProcessRunning(target))
            {
                return CreateFailure(
                    PerformanceCaptureStatus.TargetExited,
                    "Выбранное приложение закрылось до начала измерения.",
                    target,
                    elevated,
                    tool.Path,
                    csvPath);
            ...
            string sessionName = ProductBrand.ProductFileName + "_" +
            ...
            while (!process.HasExited)
            {
                ...
                Report(
                    ...
                    waiting
                        ...
                        : "Измеряем плавность выбранного приложения.");
            ...
            string toolPath)
        {
            ...
            try
            {
                ...
                if (selected == null || selected.FrameTimes.Count == 0)
                {
                    return CreateFailure(
                        PerformanceCaptureStatus.InvalidCapture,
                        "В файле PresentMon нет корректных кадров выбранного процесса.",
                        target,
                        elevated,
                        toolPath,
                        csvPath);
                ...
                double onePercentLowFps = CalculateOnePercentLowFps(sorted);
            ...
            IList<double> sortedFrameTimes)
        {
            ...
            int slowFrameCount = Math.Max(
                1,
                (int)Math.Ceiling(sortedFrameTimes.Count * 0.01));
            int firstSlowFrame = sortedFrameTimes.Count - slowFrameCount;
            double slowFrameTimeTotal = 0.0;
            for (int index = firstSlowFrame;
                index < sortedFrameTimes.Count;
                index++)
            {
                slowFrameTimeTotal += sortedFrameTimes[index];
            }

            double meanSlowFrameTime = slowFrameTimeTotal / slowFrameCount;
            return meanSlowFrameTime > 0.0
                ? 1000.0 / meanSlowFrameTime
                : 0.0;
        }
        ...
        private static ToolResolution ResolveAndValidateTool()
        {
            ...
            return new ToolResolution
            {
                ...
                Message = foundInvalid
                    ? invalidMessage
                    : "Компонент PresentMon не установлен. Переустановите приложение."
            };
        }
            ...
            string programFiles)
        {
            ...
            foreach (string productDirectory in new[]
            {
        ...
        private static void AddRunningTargets(
            ICollection<PerformanceTargetProcess> candidates)
        {
            Process[] processes;
            try
            {
        ...
        private static bool IsSameTargetProcessRunning(
            PerformanceTargetProcess target)
        {
            if (target == null ||
                target.ProcessId <= 0 ||
        ...
        private static void ValidateProtectedCaptureDirectory(string path, string name)
        {
                ...
                (directory.Attributes & FileAttributes.ReparsePoint) != 0)
            {
                throw new InvalidOperationException(
                    ...
                    " отсутствует или не прошла проверку безопасности. Переустановите приложение.");
            }
        }
        ...
        private static void ValidateElevatedCaptureFile(string path)
        {
            ...
            if (!file.Exists ||
                ...
                !Regex.IsMatch(
                    file.Name,
                    "^" + Regex.Escape(ProductBrand.ProductFileName) +
                    @"-PresentMon-[0-9a-f]{32}\.csv$",
