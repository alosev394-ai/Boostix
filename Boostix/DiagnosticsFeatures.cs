...
namespace Boostix
{
    ...
    internal static class DiagnosticSessionHistory
    {
        ...
        public static List<BoostSessionReport> LoadRecent(int maximumCount)
        {
            int count = Math.Max(
                0,
                Math.Min(MaximumSessionCount, maximumCount));
            if (count == 0)
            {
                return new List<BoostSessionReport>();
            }

            var reports = new Dictionary<string, BoostSessionReport>(
                StringComparer.OrdinalIgnoreCase);
            ...
            {
                if (!BoostSessionReportStore.IsSafeKnownSessionsDirectory(
                        directory))
                {
                    continue;
                }
                ...
                foreach (BoostSessionReport report in
                    LoadRecentFromDirectory(directory, count))
                {
                    BoostSessionReport existing;
                    if (!reports.TryGetValue(report.SessionId, out existing) ||
                        report.StartedUtc > existing.StartedUtc)
                    {
    ...
    internal static class DiagnosticExportBuilder
    {
            ...
            string notes)
        {
            ...
            builder.AppendLine(
                ProductBrand.ProductName.ToUpperInvariant() +
            ...
            int index)
        {
            ...
            builder.AppendLine("  PeakTargetWorkingSetBytes=" +
                Format(report.PeakTargetWorkingSetBytes));
            builder.AppendLine("  PeakTargetPrivateBytes=" +
                Format(report.PeakTargetPrivateBytes));
            ...
            builder.AppendLine("  TargetName=" +
                SanitizeText(report.TargetName));
            builder.AppendLine("  TargetCrashCode=" +
                SanitizeText(report.TargetCrashCode));
            builder.AppendLine("  TargetCrashModule=" +
                SanitizeText(report.TargetCrashModule));
            builder.AppendLine("  TargetCrashOffset=" +
                SanitizeText(report.TargetCrashOffset));
            builder.AppendLine("  TargetCrashUtc=" +
                (report.TargetCrashUtc.HasValue
                    ? report.TargetCrashUtc.Value.ToString(
                        "o",
                        CultureInfo.InvariantCulture)
                    : string.Empty));
