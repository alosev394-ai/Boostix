...
namespace Boostix
{
    ...
    internal sealed class BoostPerformanceResult
    {
        ...
        public string ComparisonContextKey;
    ...
    internal sealed class BoostSessionReport
    {
        ...
        public long PeakTargetWorkingSetBytes;
        ...
        public string TargetCrashModule;
        public string TargetCrashOffset;
        public DateTime? TargetCrashUtc;
        ...
        public BoostSessionReport Clone()
        {
            ...
            if (Performance != null)
            {
                clone.Performance = new BoostPerformanceResult
                {
                    ...
                    ComparisonContextKey = Performance.ComparisonContextKey,
    ...
    internal static class BoostSessionReportStore
    {
        ...
        internal static string LegacyStateDirectory
        {
            get
            {
                return Path.Combine(
                    Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData),
                    ProductBrand.LegacyDataDirectoryName);
            }
        }

        internal static string LegacySessionsDirectory
        {
            get { return Path.Combine(LegacyStateDirectory, "Sessions"); }
        }
        ...
        {
            BoostSessionReport report = LoadKnownReport(
                StateDirectory,
                "last-session.report");
            return report ?? LoadKnownReport(
        ...
        private static string Serialize(BoostSessionReport report)
        {
            ...
            lines.Add("Version=" +
                BoostSessionReport.CurrentVersion.ToString(
                    CultureInfo.InvariantCulture));
            lines.Add("SessionId=" + Encode(report.SessionId));
            lines.Add("Trigger=" + Encode(report.Trigger));
            lines.Add("Status=" + Encode(NormalizeLegacyStatus(report.Status)));
            ...
            lines.Add("PeakTargetWorkingSetBytes=" +
                report.PeakTargetWorkingSetBytes.ToString(CultureInfo.InvariantCulture));
            lines.Add("PeakTargetPrivateBytes=" +
                report.PeakTargetPrivateBytes.ToString(CultureInfo.InvariantCulture));
            ...
            lines.Add("TargetCrashCode=" + Encode(report.TargetCrashCode));
            lines.Add("TargetCrashModule=" + Encode(report.TargetCrashModule));
            lines.Add("TargetCrashOffset=" + Encode(report.TargetCrashOffset));
            lines.Add("TargetCrashUtc=" + (report.TargetCrashUtc.HasValue
                ? report.TargetCrashUtc.Value.ToString("o", CultureInfo.InvariantCulture)
                : string.Empty));
            lines.Add("TargetName=" + Encode(report.TargetName));
            ...
            if (report.Performance != null)
            {
                ...
                lines.Add("PerformanceComparisonContext=" +
                    Encode(report.Performance.ComparisonContextKey));
        ...
        private static BoostSessionReport Deserialize(string[] lines)
        {
            ...
            var report = new BoostSessionReport
            {
                ...
                Status = NormalizeLegacyStatus(Decode(GetValue(values, "Status"))),
                StartedUtc = startedUtc,
                TargetCrashCode = DecodeFirst(
                    values,
                    "TargetCrashCode",
                    "GameCrashCode"),
                TargetCrashModule = DecodeFirst(
                    values,
                    "TargetCrashModule",
                    "GameCrashModule"),
                TargetCrashOffset = DecodeFirst(
                    values,
                    "TargetCrashOffset",
                    "GameCrashOffset"),
                ...
                TargetName = DecodeFirst(values, "TargetName", "GameName"),
            ...
            if (TryParseLongFirst(
                    values,
                    "PeakTargetWorkingSetBytes",
                    "PeakGameWorkingSetBytes",
                    out longValue))
            {
                report.PeakTargetWorkingSetBytes = Math.Max(0, longValue);
            }
            if (TryParseLongFirst(
                    values,
                    ...
                    out longValue))
            {
                report.PeakTargetPrivateBytes = Math.Max(0, longValue);
            }
            ...
            if (TryParseDateFirst(
                    values,
                    "TargetCrashUtc",
                    "GameCrashUtc",
                    out dateValue))
            {
                report.TargetCrashUtc = dateValue;
            }
            ...
            if (bool.TryParse(GetValue(values, "PerformanceAvailable"), out performanceAvailable))
            {
                report.Performance = new BoostPerformanceResult
                {
                    ...
                    ComparisonContextKey = Decode(
                        GetValue(values, "PerformanceComparisonContext")),
        ...
        internal static bool IsSafeKnownSessionsDirectory(string path)
        {
            ...
            foreach (string stateDirectory in new[]
            {
                StateDirectory,
                LegacyStateDirectory
            })
            {
                string fullStateDirectory = SafeFullPath(stateDirectory);
                string expectedSessionsDirectory = SafeFullPath(
                    Path.Combine(stateDirectory, "Sessions"));
                if (string.IsNullOrWhiteSpace(fullStateDirectory) ||
                    string.IsNullOrWhiteSpace(expectedSessionsDirectory) ||
                    !string.Equals(
                        fullPath,
                ...
                return IsExistingDirectoryWithoutReparse(fullStateDirectory) &&
                    IsExistingDirectoryWithoutReparse(
                        expectedSessionsDirectory);
            }
            return false;
        }

        private static BoostSessionReport LoadKnownReport(
            string stateDirectory,
            string fileName)
        {
            string fullStateDirectory = SafeFullPath(stateDirectory);
            if (string.IsNullOrWhiteSpace(fullStateDirectory) ||
                !IsExistingDirectoryWithoutReparse(fullStateDirectory))
            {
                return null;
            }

            string path = SafeFullPath(Path.Combine(
                fullStateDirectory,
                fileName ?? string.Empty));
            string requiredPrefix = fullStateDirectory.TrimEnd(
        ...
        private static void EnsureWritableStoreDirectory()
        {
            Directory.CreateDirectory(StateDirectory);
            if (!IsExistingDirectoryWithoutReparse(StateDirectory))
            {
                throw new IOException(
                    "The " + ProductBrand.ProductName +
                    " data directory is not a regular directory.");
            }

            Directory.CreateDirectory(SessionsDirectory);
            if (!IsExistingDirectoryWithoutReparse(SessionsDirectory))
            {
                throw new IOException(
                    "The " + ProductBrand.ProductName +
        ...
        private static bool IsExistingDirectoryWithoutReparse(string path)
        {
            try
            {
        ...
        private static string DecodeFirst(
            IDictionary<string, string> values,
            string currentKey,
            string legacyKey)
        {
            string currentValue;
            if (values.TryGetValue(currentKey, out currentValue))
            {
                return Decode(currentValue);
            }
            return Decode(GetValue(values, legacyKey));
        }
        ...
        private static bool TryParseLongFirst(
            IDictionary<string, string> values,
            string currentKey,
            string legacyKey,
            out long value)
        {
            return values.ContainsKey(currentKey)
                ? TryParseLong(values, currentKey, out value)
                : TryParseLong(values, legacyKey, out value);
        }
        ...
        private static bool TryParseDateFirst(
            IDictionary<string, string> values,
            string currentKey,
            string legacyKey,
            out DateTime value)
        {
            return values.ContainsKey(currentKey)
                ? TryParseDate(values, currentKey, out value)
                : TryParseDate(values, legacyKey, out value);
        }
