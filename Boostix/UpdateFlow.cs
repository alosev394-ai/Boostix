...
namespace Boostix
{
    ...
    internal sealed class UpdateFlowOverlay : Grid
    {
        ...
        private const string ManifestUrl =
            "https://raw.githubusercontent.com/alosev394-ai/Boostix/main/update-v2.json";
        ...
        private const string RepositoryHeadUrl =
            "https://api.github.com/repos/alosev394-ai/Boostix/git/ref/heads/main";
        ...
        // compatibility name.
        private const string LegacyInstallerUrlPrefix =
            ...
            FontFamily boldFont)
        {
            ...
            AutomationProperties.SetName(this, "Проверка обновлений Boostix");
        ...
        private static UpdateManifest ParseAndValidateManifest(byte[] payload)
        {
            ...
            JsonValue boostixInstallerValue = null;
            JsonValue boostixShaValue = null;
            JsonValue boostixSizeValue = null;
            bool hasBoostixInstaller = root.ContainsKey("boostixInstallerUrl");
            bool hasBoostixSha = root.ContainsKey("boostixSha256");
            bool hasBoostixSize = root.ContainsKey("boostixSize");
            if (hasBoostixInstaller || hasBoostixSha || hasBoostixSize)
            {
                if (!hasBoostixInstaller || !hasBoostixSha || !hasBoostixSize)
                {
                    throw new InvalidDataException(
                        "Boostix installer metadata must be complete.");
                }
                boostixInstallerValue = RequireField(
                    root,
                    "boostixInstallerUrl",
                    JsonValueKind.String);
                boostixShaValue = RequireField(
                    root,
                    "boostixSha256",
                    JsonValueKind.String);
                boostixSizeValue = RequireField(
                    root,
                    "boostixSize",
            ...
            string expectedLegacyInstallerUrl =
                LegacyInstallerUrlPrefix + version + ".exe";
            if (!string.Equals(
                installerValue.Text,
                expectedLegacyInstallerUrl,
                StringComparison.Ordinal))
            {
                throw new InvalidDataException(
                    "Legacy installer URL is outside the trusted compatibility path.");
            }
            ValidateInstallerAddress(
                installerValue.Text,
                expectedLegacyInstallerUrl);

            string selectedInstallerUrl = installerValue.Text;
            if (boostixInstallerValue != null)
            {
                string expectedBoostixInstallerUrl =
                    InstallerUrlPrefix + version + ".exe";
                if (!string.Equals(
                    boostixInstallerValue.Text,
                    expectedBoostixInstallerUrl,
                    StringComparison.Ordinal))
                {
                    throw new InvalidDataException(
                        "Boostix installer URL is outside the trusted release path.");
                }
                ValidateInstallerAddress(
                    boostixInstallerValue.Text,
                    expectedBoostixInstallerUrl);
                selectedInstallerUrl = boostixInstallerValue.Text;
            }
            ...
            string selectedSha256 = sha256;
            long selectedSize = size;
            if (boostixInstallerValue != null)
            {
                if (!IsSha256(boostixShaValue.Text))
                {
                    throw new InvalidDataException(
                        "Invalid Boostix installer SHA-256.");
                }
                long boostixSize;
                if (!TryParseStrictLong(
                        boostixSizeValue.Text,
                        out boostixSize) ||
                    boostixSize <= 0 ||
                    boostixSize > InstallerMaximumBytes)
                {
                    throw new InvalidDataException(
                        "Invalid Boostix installer size.");
                }
                selectedSha256 = boostixShaValue.Text;
                selectedSize = boostixSize;
            }
            ...
            string expectedAddress)
        {
            ...
            if (!string.Equals(
                    ...
                    expectedAddress,
                    StringComparison.Ordinal) ||
                !Uri.TryCreate(address, UriKind.Absolute, out installerUri) ||
                !string.Equals(
                    installerUri.Scheme,
                    Uri.UriSchemeHttps,
                    StringComparison.Ordinal) ||
        ...
        private async Task DownloadAndLaunchUpdateAsync()
        {
            ...
            if (updateLock == null)
            {
                ShowRetry("Обновление уже скачивается в другом окне Boostix. Закройте лишнее окно и повторите попытку.");
                return;
            }
            ...
            string localApplicationData)
        {
            ...
            try
            {
                ...
                string directory = Path.GetFullPath(
                    Path.Combine(root, ProductBrand.DataDirectoryName));
            ...
            IProgress<UpdateProgressInfo> progress)
        {
            ...
            try
            {
                ...
                using (FileStream verificationStream = OpenInstallerForVerification(installerPath))
                {
                    ...
                    string safeWorkingDirectory = Environment.GetFolderPath(
                        Environment.SpecialFolder.System);
                    if (string.IsNullOrWhiteSpace(safeWorkingDirectory) ||
                        !Directory.Exists(safeWorkingDirectory))
                    {
                        throw new InvalidOperationException(
        ...
        private static string CreateUniqueDownloadDirectory()
        {
            ...
            for (int attempt = 0; attempt < 8; attempt++)
            {
                string candidate = Path.Combine(
                    root,
                    "Boostix.Update." + Guid.NewGuid().ToString("N"));
        ...
        private void ShowChecking()
        {
            ...
            BuildWorkingScreen(
                "ПРОВЕРКА ОБНОВЛЕНИЙ",
                "Проверяем актуальную версию Boostix.",
        ...
        private void ShowUpdateRequired(SemanticVersion currentVersion, SemanticVersion newVersion)
        {
            ...
            StackPanel header = BuildHeader(
                "ДОСТУПНО ОБНОВЛЕНИЕ",
                "Для продолжения необходимо обновить Boostix.");
            Grid.SetRow(header, 0);
            cardContent.Children.Add(header);
